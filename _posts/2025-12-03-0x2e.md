...
The fix is simple. Each time the binary runs, it reads the original, unmutated code from disk and mutates it fresh. Yeah, it seems like a copy, but it’s not it's different, and what we’re talkin’ about here will make more sense as we go. We create a new generation from a clean source. Whatever happened in the previous run is gone. Fuck it. Each execution starts clean. The original code gets mutated in memory, mapped into an executable view, and then we jump to its entry point.
